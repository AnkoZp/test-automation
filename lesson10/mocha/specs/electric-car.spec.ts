...
describe('ElectricCar class tests', () => {
    ...
    before(() => {
        console.log(`before globalThis.minCharg = ${globalThis.minCharg}`);
        globalThis.minCharg = 20;
    });
    ...
    it('must be charged to 100%?', () => {
        if (tesla.batteryLevel < globalThis.minCharg!) {
            tesla.charge();
            expect(tesla.batteryLevel).to.equal(100);
        }
        else {
            console.log(`Skipping charge test, battery level is above minCharg (${globalThis.minCharg})`);
        }
    });

    it('should discharge, but not below 0%', () => {
        tesla.discharge(30);
        console.log(`globalThis.minCharg = ${globalThis.minCharg}`);
        expect(tesla.batteryLevel).to.equal(globalThis.minCharg);
        tesla.discharge(100);
