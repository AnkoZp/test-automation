...
export class PlaywrightApiService implements IApiService<APIResponse> {
    ...
    public async getById(uri: string, id: string, headers?: Record<string, string>): Promise<APIResponse> {
        const defaultHeaders: Record<string, string> = {
            ...this.getAuthHeader(),
            ...{
                'Content-Type': 'application/json',
        ...
        const requestContext = await this.getRequestContext();
        const relativePath = this.convertToRelativePath(`${uri}/${id}`);
        const response = await requestContext.get(relativePath, {
    ...
    public async delete(uri: string, id: string, headers?: Record<string, string>): Promise<APIResponse> {
        const defaultHeaders: Record<string, string> = this.getAuthHeader();
        const requestContext = await this.getRequestContext();
        const relativePath = this.convertToRelativePath(`${uri}/${id}`);
        return await requestContext.delete(relativePath, { headers: { ...defaultHeaders, ...headers } });
    }
